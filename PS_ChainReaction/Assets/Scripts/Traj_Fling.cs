...
public class Traj_Fling : MonoBehaviour
{
    ...
    private Rigidbody _rb; //Rigidbody who's taking Force
    ...
    {   
        Collider[] hits = Physics.OverlapSphere(position, _collisionCheckRadius);
        if (hits.Length > 0)
        {
    ...
    void Fling() //Click mouse to start trajectory, longer trajectory means more force, up to 3f distance or 500 force
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            Vector3 mousePosNoY = new Vector3 (mousePos.x, .5f, mousePos.z);
            _castLine.SetPosition(0, mousePosNoY); //Set Initial position of CastLine
            StartCoroutine(FlingCalc(mousePosNoY)); //Begin Coroutine to continually update drawing CastLine towards the current mouse position
        }
